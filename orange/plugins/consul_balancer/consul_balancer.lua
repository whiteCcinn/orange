...
local function _persistServiceInDb(service_descriptor, res)
    local store = orange.data.store
    dao.updatreOrCreateServiceCache(store, service_descriptor, res)
end

local function _getResultCacheByDb(service_descriptor, res)
    local store = orange.data.store
    local ret = dao.getServiceCacheByService(store, service_descriptor)

    if ret == nil then
        ngx.log(ngx.ERR, "consul.balancer: _getResultCacheByDb is nil")
        return nil
    end
...
local function _watch(premature, service_descriptor)
    ...
    ngx.log(
        ngx.NOTICE,
        "consul.balancer: started watching for changes in ",
    ...
    local tLast = ostime() + orange.data.config.consul.gc_interval or 90
    local pf = orange.data.config.consul.persistence_frequency or 10
    while t do
        local res, err = _check(service_descriptor, service_index)
        _M._check_count = _M._check_count + 1
        if res == nil then
            ngx.log(
                ngx.ERR,
                "consul.balancer: failed while watching for changes in ",
                service_descriptor.name,
                " retry scheduled"
            )
            -- 从数据库拿到res
            local res_db = _getResultCacheByDb(service_descriptor)
            if res_db == nil then
                _timer(WATCH_RETRY_TIMER, _watch, service_descriptor)
                return nil
            end
            res = json.decode(res_db["value"])
        end

        if math.fmod(_M._check_count, pf) == 0 then
            ngx.log(
                ngx.NOTICE,
                "consul.balancer: persist service in db ",
                "server:",
                service_descriptor.name,
                " key: ",
                service_descriptor.id
            )
            _persistServiceInDb(service_descriptor, res)
        end
        ...
        if service_descriptor.log_consul then
            ngx.log(
                ngx.INFO,
                "consul.balancer: persisted service ",
                service_descriptor.name,
                "(",
                service_descriptor.id,
                ") index: ",
...
function _M.round_robin(srv)
    ...
    if not ok then
        ngx.log(
            ngx.ERR,
            "consul.balancer: failed to set the current peer",
            upstream["address"],
            ":",
            upstream["port"],
            ",",
